...
function upload_method($html_parameter, $uploadDir) {
    ...
    elseif (isset($_POST[$html_parameter . "_sra_accession"])) {
        ...
        $sraCommand = "/home/inseqt/edirect/efetch -db sra -id $uploadSRAAccession -format runinfo | tail -1 | cut -d, -f16";
        $ngs_layout = shell_exec($sraCommand);
        $ngs_layout = trim($ngs_layout);

        if ($ngs_layout === "SINGLE") {
            $sraCommand = "mkdir -m 777 $uploadDir/$uploadSRAAccession ; "
                . "/home/inseqt/RNASEQ/sratoolkit.3.0.2-ubuntu64/bin/prefetch $uploadSRAAccession -T sra -O $uploadDir -q 2>/dev/null ; "
                . "/home/inseqt/RNASEQ/sratoolkit.3.0.2-ubuntu64/bin/fasterq-dump $uploadDir/$uploadSRAAccession -O $uploadDir/$uploadSRAAccession -f --split-spot -q 2>/dev/null ; "
                . "gzip -f $uploadDir/$uploadSRAAccession/{$uploadSRAAccession}*.fastq ; "
                . "rm -rf $uploadDir/$uploadSRAAccession/{$uploadSRAAccession}*.sra*";
            shell_exec($sraCommand);

            // Define file paths for single files
            $file1 = $sraDir . $uploadSRAAccession . ".fastq.gz";
            $file2 = "" ;
        } elseif ($ngs_layout === "PAIRED") {
            $sraCommand = "mkdir -m 777 $uploadDir/$uploadSRAAccession ; "
                . "/home/inseqt/RNASEQ/sratoolkit.3.0.2-ubuntu64/bin/prefetch $uploadSRAAccession -T sra -O $uploadDir -q 2>/dev/null ; "
                . "/home/inseqt/RNASEQ/sratoolkit.3.0.2-ubuntu64/bin/fasterq-dump $uploadDir/$uploadSRAAccession -O $uploadDir/$uploadSRAAccession -f --split-files -q 2>/dev/null ; "
                . "gzip -f $uploadDir/$uploadSRAAccession/{$uploadSRAAccession}*.fastq ; "
                . "rm -rf $uploadDir/$uploadSRAAccession/{$uploadSRAAccession}*.sra*";
            shell_exec($sraCommand);
        ...
        if (file_exists($file1)) {
            if ($file2 && file_exists($file2)) {
                echo "<p>Files have been successfully processed and stored.</p>";
                $uploadedFilePaths[] = $file1 ;
                $uploadedFilePaths[] = $file2 ;
            } else {
                echo "<p>File has been successfully processed and stored.</p>";
                $uploadedFilePaths = [$file1] ;
            }
        } else {
...
function run_virae($reads_file_path, $ref_file_path, $output_dir) {
    ...
    $alignment_stringency    = isset($_POST['alignment_stringency'])
    ? intval($_POST['alignment_stringency'])
    : 30;

    $mapping_quality_cutoff  = isset($_POST['mapping_quality_cutoff'])
    ? intval($_POST['mapping_quality_cutoff'])
    : 0;

    // Construct the command based on the number of reads files
    if (count($reads_file_path) == 1) {
        // SINGLE-END scenario
        $command = "/home/inseqt/COSTAS_CON/TOOLS/SCRIPTS/ViRAE.sh -1 {$reads_file_path[0]} -r $ref_file_path -o $output_dir -l $alignment_stringency -q $mapping_quality_cutoff";
    } elseif (count($reads_file_path) == 2) {
        // PAIRED-END scenario
        $command = "/home/inseqt/COSTAS_CON/TOOLS/SCRIPTS/ViRAE.sh -1 {$reads_file_path[0]} -2 {$reads_file_path[1]} -r $ref_file_path -o $output_dir -l $alignment_stringency -q $mapping_quality_cutoff";
    } 
