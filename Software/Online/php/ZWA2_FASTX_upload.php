...
function upload_method($html_parameter, $uploadDir) {
    ...
    elseif (isset($_POST[$html_parameter . "_dropdown_menu_options"])) {
        ...
        if ($uploadDropdownMenu == "silva_ref_file") {
            ...
            $uploadFilePath = "/mnt/14C608D4C608B7CE/blast/HYBRID_READS-ZWA/DATA/HOST_GENOMES/Homo_sapiens.GRCh38.cdna.all.fa.gz";
            return $uploadFilePath;
        }
    }
    // Check for file link
...
// Define run_zwa function
function run_zwa($reads_file_path, $ref_file_path, $output_dir) {
    echo "<h1>ZWA2 execution result</h1>";
    echo "<p>Running ZWA2 now, please wait...</p>";

    $command = "/home/inseqt/COSTAS_CON/TOOLS/SCRIPTS/ZWA2-NIKOLAS-LIKE-BWA_ONLY.sh -i $reads_file_path -r $ref_file_path -o $output_dir";
    $output = shell_exec($command);
    echo "<pre>$output</pre>";
    
}

// Define download_zwa_files function
function download_zwa_files($reads_file_path, $ref_file_path) {
    global $downloadDir;
...
if (file_exists($uploadReadsFilePath)) {
    ...
    if (file_exists($uploadRefFilePath) && $uploadReadsFilePath !== $uploadRefFilePath) {
        run_zwa($uploadReadsFilePath, $uploadRefFilePath, $downloadDir);
        download_zwa_files($uploadReadsFilePath, $uploadRefFilePath);
        ...
        $errorMessage = "Invalid input FASTA file! Click OK to be redirected to the ZWA2 file upload page.";
        ...
        echo "<meta http-equiv='refresh' content='1;url=https://srv-inseqt.med.duth.gr/ZWA2/HTML/ZWA2_FASTX_upload.html'>";
    }
} elseif (!file_exists($uploadReadsFilePath)) {
    $errorMessage = "Invalid input FASTQ file! Click OK to be redirected to the ZWA2 file upload page.";
